<template>
  <div>
    <div class="page-title">Tablou de bord — Contabilitate</div>
    ...
    <div v-else>
      <div class="row">
        ...
        <div class="flex xs12 sm6 md3"><kpi label="Rată de încasare" :value="data.collectionRate + '%'" icon="percent" /></div>
        <div class="flex xs12 sm6 md3"><kpi label="Total restant" :value="fmt(data.overdueAmount) + ' RON'" :sub="data.overdueCount + ' facturi'" icon="warning" color="danger" /></div>
      </div>
      ...
      <va-card class="mt-4">
        ...
        <va-card-content>
          <va-data-table :items="data.overdueInvoices" :columns="cols" no-data-html="Nicio factură restantă.">
...
export default {
  ...
  setup() {
    ...
    const load = async () => {
      try { data.value = (await api.get('/dashboard/contabil')).data; }
    ...
    onMounted(load);
    return { data, loading, cols, monthlyCols, fmt, generate, generating, showMonthlyModal };
  }
};
</script>
